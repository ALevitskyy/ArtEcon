from flask import Blueprint, url_for, redirect, render_template,session,request,send_file,request,flash
...
COMMISIONS={'AUDUSD':0.00025, 'EURCHF':0.0003,'EURGBP':0.00025,'EURJPY':0.032,'EURUSD':0.00019,'GBPUSD':0.00024,
    'NZDUSD':0.0003,'USDCAD':0.0003,'USDCHF':0.00025,'USDJPY':0.02}
CURR_CODES=[("0",'AUDUSD'), ("1",'EURCHF'), ("2",'EURGBP'), ("3",'EURJPY'), ("4",'EURUSD'), ("5",'GBPUSD'),
    ("6",'NZDUSD'), ("7",'USDCAD'), ("8",'USDCHF'), ("9",'USDJPY')]
CANDLE_TYPES=[("5M","5M"),("30M","30M"),("1H","1H"),("4H","4H"),("8H","8H"),("1D","1D")]
LEVERAGES=[("1","x1"),("5","x5"),("20","x20"),("50","x50"),("100","x100"),("200","x200"),("400","x400")]
BUY_SELL=[("BUY","BUY"),("SELL","SELL")]

class InitForm(Form):
    init_cash=IntegerField('Initial Cash',default=200,validators=[NumberRange(min=1,message="The field only accepts positive integer values")])
    leverage=SelectField("Leverage", choices=LEVERAGES)
    lot_size=IntegerField("Lot Size",default=10000,validators=[NumberRange(min=1,message="The field only accepts positive integer values")])
    date_time=DateTimeField('Start Date',validators=[DateRange(
                                                               min=datetime(2015, 1, 1),
                                                               max=datetime(2018, 6, 29))])
class MakeDealForm(Form):
    currency2=SelectField('Currency', choices=CURR_CODES)
    lot_number=DecimalField("Number of lots",default=1,validators=[NumberRange(min=0.00000001,message="The field only accepts positive values")])
    buy_sell=SelectField('BUY/SELL', choices=BUY_SELL)
    
class CloseDealForm(Form):
    id=StringField("id")
class CandleRequestForm(Form):
    date_time=DateTimeField('Start Date',default=pd.to_datetime("2016-01-01 09:00:00"),validators=[DateRange(
                                                               min=datetime(2015, 1, 1),
                                                               max=datetime(2018, 6, 29))])
    currency=SelectField('Currency', choices=CURR_CODES)
    candle_type=SelectField('Candle', choices=CANDLE_TYPES)
    candle_number=IntegerField('Number of candles',default=300,validators=[NumberRange(min=1,message="The field only accepts positive integer values")])
...
def get_current_price(currency):
    ...
    data=pd.read_csv(DATA_PATH+str(file_number)+
                                               ".csv",**pandas_params)
...
def get_free_margin():
    ...
    for i in session["log"]:
        ...
        if i["status"]=="Active":
            session["free_cash"]-=i["size"]*session["lot_size"]/session["leverage"]

def calculate_deal_profit(log_record):
    if log_record["order"]=="BUY":
        difference=(log_record["current_price"]-log_record["initial_price"]-\
            COMMISIONS[log_record["currency"]])/log_record["current_price"]
    else:
        difference=(log_record["initial_price"]-log_record["current_price"]-\
            COMMISIONS[log_record["currency"]])/log_record["current_price"]
...
def getData():
    ...
    if form.validate_on_submit():
        ...
        index=index.index.get_loc(pd.to_datetime(date),
                                                        method='nearest')
        if index<number:
            return jsonify(errorrs="There is not enough historical data in database to process your request. The records in database starts on 1st of January 2015, and there is no data prior")
        if index+skipping>=260884:
            return jsonify(errorrs="There is no data in the database past the current date of simulation, so you would not be able to continue simulation")
        index=int(currency)*260885+index
        index+=skipping
        ...
        while candles_left>2500:
            ...
            file_number-=2500
            data2=pd.read_csv(DATA_PATH+str(file_number)+
                            ".csv",**pandas_params)
...
def initiate_session():
    ...
    if init_form.validate_on_submit():
        ...
        session["skip_date"]=0
        session["log"]=[]
        return jsonify(total_cash=session["total_cash"],
...
def make_deal():
    ...
    if make_deal_form.validate_on_submit():
        ...
        try:
            ...
            print(e)
            return jsonify(errorrs="Not enough historical data to statisfy your request")
        if float(make_deal_form.lot_number.data)*session["lot_size"]/session["leverage"]>session["free_cash"]:
            return jsonify(errorrs="You don`t have enough free margin to commence the operation")
        deal_dict=dict(id=len(session["log"]),
                       currency=[i[1] for i in CURR_CODES if i[0]==make_deal_form.currency2.data][0],
        ...
        return jsonify(log=session["log"],total_cash=session["total_cash"],
                       free_cash=session["free_cash"])
...
def close_deal():
    ...
    if close_deal_form.validate_on_submit():
        try:
            index=[i["id"] for i in session["log"]].index(int(close_deal_form.id.data))
...
def skip_date():
    ...
    if "log" in session:
        ...
        for i in session["log"]:
            if i["status"]=="Active":
                try:
                    i["current_price"],i["close_date"]=get_current_price([z[0] for z in CURR_CODES if z[1]==i["currency"]][0])
                except:
                    return jsonify(errorrs="Your skip step request exceeeds the historical data available.")
...
#margin call
        if session["free_cash"]<0:
            ...
            get_free_margin()
            return jsonify(log=session["log"],total_cash=session["total_cash"],
                           free_cash=session["free_cash"],margin_call="SHIT")
        session.modified = True
        return jsonify(log=session["log"],total_cash=session["total_cash"],
                       free_cash=session["free_cash"])
    return "skipped"
...
def load_sesh():
    if request.method == 'POST':
        ...
        if file:
            ...
            return jsonify(log=session["log"],total_cash=session["total_cash"],
                           free_cash=session["free_cash"],start_date=str(session["start_date"]),
                        skipping=session["skip_date"])
