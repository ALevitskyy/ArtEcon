...
def article_show(articlename):
    query = db.engine.execute(
        "Select file,id From Post Where url='" + articlename + "'"
    )
    ...
    query2 = db.engine.execute(
        """WITH RECURSIVE nodes_cte(comment_id, post_id, comment_author, timestamp, comment_text,parent_id,depth,path) AS (
        SELECT tn.comment_id, tn.post_id, tn.comment_author, tn.timestamp::timestamp(0), tn.comment_text,tn.parent_id, 1::INT AS depth, tn.comment_id::TEXT AS path FROM comment AS tn WHERE tn.post_id ="""
        + str(post_id)
        + """AND tn.parent_id is null UNION ALL
    ...
    if comment_form.validate_on_submit():
        if (
            comment_form.parent_id.data == "99999999999999999"
            or comment_form.parent_id.data == 99999999999999999
        ):
            entry = Comment(
                post_id=post_id,
...
def all_cater():
    tags_query = db.engine.execute(
        "Select Count(tag_id),tag_title from Tag group by tag_title order by Count(tag_id) desc"
    )
    return render_template("/all_categories.html", tags=get_tags(), caters=tags_query)
